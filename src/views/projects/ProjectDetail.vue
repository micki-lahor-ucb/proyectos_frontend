<template>
  <div>
        ...
        <div class="md:col-span-2">
          <div class="card">
            ...
            <div class="mb-4">
              ...
              <p class="bg-gray-50 p-3 rounded">{{ project.description || 'Sin descripción' }}</p>
            </div>
            <div class="flex flex-wrap gap-x-6 gap-y-2 text-sm">
              ...
              <p><strong>Creado:</strong> {{ formatDateTime(project.createdAt) }}</p>
              <p><strong>Última actualización:</strong> {{ formatDateTime(project.updatedAt) }}</p>
            </div>
          </div>
        </div>
        <div>
          <div class="card">
            ...
            <div class="mb-4">
              <p class="text-2xl font-bold text-center">{{ completedTasks }} / {{ totalTasks }}</p>
    ...
    <!-- Incluir los modales de tareas que ya teníamos -->
    <!-- ... -->
  </div>
</template>
...
const inProgressTasks = computed(() => 
  project.value?.tasks?.filter(task => task.status === 'IN_PROGRESS').length || 0
);
